...
function createText(text, x, y, size, color, weight = "Regular", width) {
...
function createRoundedRect(x, y, w, h, fill, stroke, strokeWeight, radius) {
    const rect = figma.createRectangle();
    rect.x = x;
    rect.y = y;
    rect.resize(w, h);
    rect.cornerRadius = radius !== null && radius !== void 0 ? radius : 8;
    rect.fills = [{ type: "SOLID", color: fill }];
    if (stroke) {
        rect.strokes = [{ type: "SOLID", color: stroke }];
        rect.strokeWeight = strokeWeight !== null && strokeWeight !== void 0 ? strokeWeight : 1.5;
    }
    return rect;
}
function createLine(x1, y1, x2, y2, color, weight = 2, dash) {
...
function createFrame(name, x, y, w, h, fill) {
...
// ── カード作成ヘルパー ──
function createCard(parent, x, y, w, h, headerText, headerBg, bodyBg, borderColor, bodyLines) {
    var _a;
    // カード背景
    ...
    for (const line of bodyLines) {
        const t = createText(line.text, x + 16, y + 36 + line.y, line.size, line.color, (_a = line.weight) !== null && _a !== void 0 ? _a : "Regular", w - 32);
