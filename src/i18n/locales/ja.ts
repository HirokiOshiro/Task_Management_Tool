  ...
  help: {
    ...
    sections: {
      ...
      views: {
        ...
        gantt: { name: 'ガントチャート', desc: 'タスクの開始日〜期限をバーで表示。バーのドラッグで日程移動、端のドラッグで期間調整ができます。空き領域をダブルクリックして新規タスクを作成したり、マウスドラッグで複数タスクを範囲選択して一括移動することもできます。グループ化時はヘッダーの「+」ボタンでそのカテゴリーのタスクを作成できます。「今日」ボタンで現在日にスクロール、月表示／2週間表示の切替もできます。依存関係を設定したタスク間には矢印が表示されます。' },
      ...
      dataSection: {
        ...
        openFileDesc: 'JSON または Excel（.xlsx）ファイルを選択して読み込みます。読み込み時には安全性検証が行われ、不正な構造や未対応キーを含むデータは取り込まれない場合があります。',
        saveJsonTitle: 'JSONで保存',
        saveJsonDesc: 'すべてのタスク・フィールド設定・ビュー設定をJSON形式でダウンロードします。',
        saveJsonRecommend: '推奨の保存形式',
        saveJsonNote: 'です。フィールド構成やメモなど全ての情報が保存され、復元用バックアップに最適です。',
        saveExcelTitle: 'Excelで保存',
        saveExcelDesc: 'タスクデータをExcelファイル（.xlsx）としてダウンロードします。ExcelやGoogleスプレッドシートでの開覧・共有に便利で、安全のため先頭が = / + / - / @ の文字列はテキストとして保存されます。',
      ...
      caution: {
        ...
        items: [
          ...
          { title: 'ファイルへのバックアップを推奨', desc: '自動保存はブラウザ内のみです。ブラウザのデータ削除やキャッシュクリアに加え、保存データが破損・不正と判定された場合は安全のため復元されないことがあります。重要なデータは定期的に「JSONで保存」でバックアップしてください。' },
          { title: 'デモデータの上書きに注意', desc: '「デモデータ」をクリックすると現在のデータがサンプルデータに置き換わります。大切なデータがある場合は先に保存してください。' },
          { title: '共有端末での利用', desc: '共有PCや共同端末で使用する場合、ブラウザのローカルストレージにデータが残ります。利用後はJSONで保存し、必要に応じてブラウザデータを削除してください。機密情報の常時保存は避けてください。' },
